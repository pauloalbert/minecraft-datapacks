...
# Trims index
scoreboard players set @s math_input1 312
scoreboard players set @s math_input2 322
...
execute if score @s wt_trades >= @s wt_random run tag @s add has_new_trim_trades
...
tag @s[tag=!has_new_trim_trades] remove success

# Get trade index
function wandering_trades:math/random_trade_index

# Check if trade index is in existing trades
execute at @s[tag=!success,tag=!has_new_trim_trades] as @e[type=minecraft:item,distance=..0.5,tag=trade_index_copy] run function wandering_trades:check_existing_trades

# Add trade to Trade Index  
execute at @s[tag=success,tag=!has_new_trim_trades] as @e[type=minecraft:item,distance=..0.5,tag=trade_index] run function wandering_trades:append_trade_index_to_list

# Add trade to wandering trader if it has not been added already
execute as @s[tag=success,tag=!has_new_trim_trades] at @s run function wandering_trades:add_trade